...
- **自动扫描**：全库扫描为后台作业（POST 启动 / GET 轮询），从视频根目录自动识别番剧、剧集与季号（支持 `[01]`、`[01v2]`、`EP01`、`[E01]`、`_01_`、`第1集`/`第０１集`、`S1E1` 等常见命名格式）；无集号但标题够强的文件先 Search，仅当 Bangumi 为单集/电影平台或文件名含剧场版标记时才写 ep1；Search 主名对不上时用 infobox 别名重打分（阈值不变）；未识别结果持久保存
